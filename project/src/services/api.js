...
class ApiService {
  ...
  async request(endpoint, options = {}) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.message || responseData.error || 'An unknown error occurred');
      }
  ...
  async register(userData) {
    ...
    // Generate username from email, replacing special characters with underscores to match backend validation
    formData.append('username', userData.email.split('@')[0].replace(/[^a-zA-Z0-9_]/g, '_'));
  ...
  async getRelatedStories(storyId) {
    return this.request(`/stories/related/${storyId}`);
  }
